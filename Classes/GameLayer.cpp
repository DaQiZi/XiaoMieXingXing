...
void GameLayer::initData(){
	...
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(GameLayer::propUseComplate), PROP_COMPLATE, NULL);

	_currentScore = MyUserData::getInstance()->getCurrentScore();
	...
	matrix = nullptr;
	StarMatrix::gameLayer = this;

	_isPass = false;

}

void GameLayer::checkLabelStatus(float){
...
void GameLayer::initBG(){
	...
	//目标标签
	auto targetSpr = Sprite::create("texture/running_dst.png");
	targetSpr->setPosition(Vec2(_winSize.width*0.35f,_winSize.height*0.97f));
	this->addChild(targetSpr);
	//目标分数
	_targetScoreLabel = Label::createWithCharMap("fonts/num4.png", 19, 26, '0');
	_targetScoreLabel->setPosition(Vec2(targetSpr->getPositionX() + 100, targetSpr->getPositionY()));
	_targetScoreLabel->setString(StringUtils::format("%d", _targetScore));
...
void GameLayer::propTouchJudge(Vec2 touch){
...
void GameLayer::trasitionLabel(){
	auto tipLabel = TipLabel::create(_currentLevel,_targetScore);
	tipLabel->setScale(0);
	tipLabel->setPosition(Vec2(_winSize.width/2,_winSize.height/2));
	auto action_1 = ScaleTo::create(1.0f,1.0f);
	auto actionD = DelayTime::create(0.5f);
	auto action_2 = ScaleTo::create(0.5f,0);
...
bool GameLayer::init(){
	...
	//initStarMatrix();
	//添加一个飘动感觉
	schedule(schedule_selector(GameLayer::checkLabelStatus),1.0f);
	return true;
}

void GameLayer::propUseComplate(Ref* data){
	_selectProp->setIsSelected(false);
	_selectProp->stopAllActions();
	_selectProp->setScale(1.0f);
...
void GameLayer::updateTargetScore(int score){
	_currentScore += score;
	_currentScoreLabel->setString(StringUtils::format("%d",_currentScore));
...
void GameLayer::gameComplate(){
	NotificationCenter::getInstance()->removeAllObservers(this);
	if (_currentScore>=_targetScore){
		log("game complate");

		MusicManagement::getInstance()->playLevelPass();
		//下一关
		MyUserData::getInstance()->setCurrentLevel(MyUserData::getInstance()->getCurrentLevel()+1);
...
void GameLayer::produStarMatrixAndAddListener(){
